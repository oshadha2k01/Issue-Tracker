...
const IssueForm = ({ onSubmit, onCancel, editIssue, viewMode }) => {
  ...
  const validateField = (name, value) => {
    ...
    if (
      name === "status" &&
      !["Open", "In Progress", "Testing", "Resolved", "Closed"].includes(value)
    ) {
  ...
  const handleSubmit = (e) => {
    ...
    const request = editIssue
      ? axios.put(`${API_BASE_URL}/api/issues/${editIssue._id}`, formData, {
          headers,
        })
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-12 lg:col-span-4">
        ...
        <select
          ...
          disabled={viewMode}
        >
          {["Open", "In Progress", "Testing", "Resolved", "Closed"].map(
            (option) => (
              <option key={option} value={option}>
                {option}
              </option>
